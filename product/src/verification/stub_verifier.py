"""M2 stub Verifier: digest-equality verification of a published Result.
...
defined by the M2 plan (§3). Real evidence policy is M5.
"""

from __future__ import annotations

from kernel.protocol import RecordRef
from kernel.protocol_v1 import CoverageEntryV1, TaskV1, VerificationV1
...
def stub_verify(
    ...
    digests match, else UNSATISIFIED with no evidence. Verdict is PASS iff
    coverage is non-empty and every entry is SATISFIED, else FAIL (this stub
    ...
    if verdict != "PASS":
        findings = tuple(
            f"criterion unsatisfied: {criterion}"
            for criterion in task.acceptance_criteria
        )
