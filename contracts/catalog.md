...
| Attempt Packet | Context Compiler candidate | Kernel | execution contract bound to workflow/task/context/workspace/runtime envelope and capability grant |
| Role Capability Grant | Policy/Kernel candidate | Kernel/Host | bounded execution capabilities for an exact role/attempt; cannot be widened by the role itself |
...
| Runtime Observation | Runtime adapter/Host observation | Kernel | observed runtime fact bound to Attempt and output snapshot; never completion authority |
